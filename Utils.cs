...
namespace BinaryEditor
{
    ...
    {
        public static void CollectCSV(string dir)
        {
            string[] files = Directory.GetFiles(dir, "*.csv", SearchOption.AllDirectories);
            using (StreamWriter writer = new StreamWriter(dir + "_TBL1.csv", false, System.Text.Encoding.UTF8))
            {
                for (int i = 0; i < files.Length; i++)
                    writer.Write(File.ReadAllText(files[i]) + "\n");
            }
        }
        public static void CopyTxtFiles(string sourcePath)
        {
            // Определяем путь для новой папки
            string copyFolderPath = Path.Combine(sourcePath, "copyed");
            ...
            // Копируем каждый файл в папку copyed
            foreach (string filePath in txtFiles)
            {
                string fileName = Path.GetFileName(filePath);
                string destFile = Path.Combine(copyFolderPath, fileName);
                File.Copy(filePath, destFile, true); // true - перезаписать, если файл уже существует
            }
        }
        public static List<string> Names => File.ReadAllLines("Names.txt").ToList();
        public static string GetName(short Bytes)
        {
            if (Bytes >= Names.Count)
                return $"{Bytes:x4}";
            return Names[Bytes];
        }
        public static string[] GetSJStrings(string file)
        {
            List<string> strings = [];
            string[] st = File.ReadAllLines(file, Encoding.UTF8);
            for (int i = 0; i < st.Length; i++)
            {
                if (st[i].StartsWith("//"))
                {
                    while (st[i].StartsWith("//"))
                    {
                        strings.Add(st[i]);
                        i++;
                    }
                ...
                else if (st[i] == "###")
                {
                    ...
                    while (st[i] != "###")
                    {
                        ...
                        if (i >= st.Length - 1)
                        {
                            strings.Add(tmp.Substring(0, tmp.Length - 1));
        ...
        public static (string[], int[]) GetStrings(string file, Encoding enc)
        {
            string[] stand = File.ReadAllLines(file, enc);
            List<string> sorted = [];
            List<int> ID = [];
            for (int i = 0; i < stand.Length; i++)
            {
                if (stand[i].StartsWith("###"))
                {
                    ID.Add(Convert.ToInt32(stand[i].Substring(3, 4), 10));
                    continue;
                }
                string conv = "";
                while (!stand[i].StartsWith("###"))
                {
                    conv += stand[i];
                    if (i == stand.Length - 1)
                        break;
                    if (!stand[i].EndsWith(">") && stand[i + 1] != "<next>" && stand[i + 1] != "<choice>")
                        conv += "<lf>";
                    i++;
                }
                if (i == stand.Length - 1)
            ...
            return (sorted.ToArray(), ID.ToArray());
        }
        public static int SkipBytes(BinaryReader reader, byte skipByte, int? needPlussing = null)
        {
            List<byte> sfa = [];
            try
            {
                while (reader.ReadByte() == skipByte)
                    sfa.Add(1);
            }
            catch
            {
            ...
            try
            {
                return sfa.Count + (int)needPlussing;
            }
            catch
            {
                return sfa.Count;
        ...
        public static byte[] ParseSMTName(string Name)
        {
            bool isSucces = false;
            List<byte> name = [];
            ...
            try
            {
                ...
                {
                    if (Names[i].Split('\t')[2] == Name)
                    {
                        isSucces = true;
                        name.AddRange(new byte[] { Convert.ToByte(Names[i].Split('\t')[0], 16), Convert.ToByte(Names[i].Split('\t')[1], 16) });
                    }
                }
                if (!isSucces)
                    name.AddRange(new byte[] { Convert.ToByte(Name.Split(' ')[0], 16), Convert.ToByte(Name.Split(' ')[1], 16) });
            }
            catch
            {
                name.AddRange(new byte[] { Convert.ToByte(Name.Split(' ')[0], 16), Convert.ToByte(Name.Split(' ')[1], 16) });
